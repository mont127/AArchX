/* div/idiv coverage: every pairing of a set of 64- and 32-bit dividends and
 * divisors (zero divisors and the INT_MIN / -1 overflow excluded, since both
 * raise #DE), then the 128/64 forms driven through inline asm -- an unsigned
 * divq with a nonzero high half, and a signed cqto + idivq -- and the
 * sign-extension family cbw/cwde/cdqe and cwd/cdq/cqo.  Results are folded
 * into one checksum; golden from the native run. */
#include "gsys.h"
static g_u64 acc;
