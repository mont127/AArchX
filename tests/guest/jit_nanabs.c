 ...
 *
 * The three cases are: (a) S = 0*inf produces a NaN that flows into D through a
 * propagating add, and both are observed; (b) the same with D reloaded from
 * memory before the batch ends, so S must be checked itself; and (c) a consumer
 * of maxps, which on x86 does not propagate NaN at all (max(NaN, x) = x).
 */
#include "gsys.h"
static void put(const char *tag, g_u64 v) { g_puts(tag); g_puthex64(v); }
static float fa[4] __attribute__((aligned(16))) = { 0.f, 1.f, 2.f, 3.f };
static float finf[4] __attribute__((aligned(16))) = { 1.f/0.f, 1.f, 1.f, 1.f };
