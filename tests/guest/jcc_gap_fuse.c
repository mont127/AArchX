...
/* libSystem's backward UTF-8 scan: the gap writes the compare's index
 * register.  The NZCV-forwarding fallback used to re-load the byte with the
 * new index and spun forever. */
...
int main(int argc, char **argv, char **envp)
{
    ...
    for (int pass = 0; pass < 3; pass++) {      /* three passes: blocks compile, then chain */
    ...
    for (int i = 0; i < 64; i++) utf[i] = (unsigned char)(i >= 40 ? 0xe0 : 0x41);   /* continuation-ish tail from 40 */
