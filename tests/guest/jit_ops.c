/* Broad integer-operation coverage, checksummed; golden from the native run.
 *
 * Shifts and rotates at counts of 0, 1, 7, 31, 32 and 63 (the counts where
 * masking, width and the count==0 flag rule all differ), not/neg/bswap, and
 * the compare-and-select shapes clang lowers to cmov.  Then every one of the
 * 16 conditions is read back with setcc after both cmp and test at all four
 * operand sizes, which is what pins the flag semantics rather than just the
 * results, and the same conditions again through cmov.
 *
 * argv[1] selects one section -- s shifts, r rotates, n the unary ops, c the
 * compares -- and no argument runs them all. */
#include "gsys.h"
static g_u64 acc;
