 ...
 *
 * The handler reads the trapping state straight out of the x86_64 mcontext
 * (__ss starts at +16, so rax is +16, rdx +40 and rip +144). The cases are
 * `xor edx,edx ; div` by zero, where rdx must read 0 in the handler even though
 * the xor was elided; `cqo ; idiv -1` with rax = INT_MIN, where rdx must read
 * sext(rax) = -1; and the 32-bit `cdq ; idivl -1` twin.
 */
#include "gsys.h"
