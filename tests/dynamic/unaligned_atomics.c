 ...
 * at +0x446, a dword lock word that is 2 mod 4.
 *
 * Four threads hammer counters at misaligned offsets with lock xadd / lock add
 * / lock inc, take a cmpxchg spinlock whose word is misaligned (released with
 * xchg, as Wine's LeaveCriticalSection does), and bump a plain counter under
 * it.  The sites cover a dword 2 mod 4, a dword crossing a 16-byte granule, a
 * qword 4 mod 8, a qword crossing a page, and the lock word at the Steam
 * offset.  Every total must be exact.
 */
#include <pthread.h>
#include <stdio.h>
#include <stdint.h>
