/*
 * The wine-free proof that the WoW64 mode switch works.
 *
 * wine-11.8 picks its WoW64 CPU backend from the NATIVE machine: ARM64 selects
 * xtajit.dll, anything else selects wow64cpu.dll.  Under ocerz wine is an
 * x86_64 Mach-O, so wine believes the native machine is AMD64 and loads its own
 * wow64cpu - the backend written for a CPU that has hardware compatibility
 * mode.  There is no BTCpuSimulate contract for an emulator to implement;
 * wow64cpu's BTCpuSimulate is a dozen x86_64 instructions and ocerz's entire
 * obligation is to execute them correctly.  So what is tested here is not an
 * interface, it is four exact instruction encodings and the register
 ...
 * in both modes.  The pair that matters most is `ff 2d disp32`: a ModRM with
 * mod=00 rm=101, which means RIP-relative in long mode and ABSOLUTE disp32 in
 * 32-bit mode - the one encoding whose meaning inverts between the two modes -
 * and it is exactly what wine's 32->64 thunk uses for every WoW64 syscall,
 * because wine fills the thunk with an absolute address.  A decoder that got
 * this backwards would send every 32-bit syscall to a wild address.
 *
 ...
 * there is no REX prefix outside long mode, and 0x40..0x4f are INC/DEC there -
 * so the invariant asserted is the strong one: the ENTIRE high half of the
 * register file is untouched from mode entry to mode exit.  One section runs a
 ...
 *
 * The LDT descriptor 0x00cffb000000ffff is the canonical flat 32-bit user code
 * descriptor: base 0, limit 0xfffff with G=1 (so 4 GB), type 0xfb = present |
 * DPL 3 | code | readable | accessed, D/B=1 and L=0.  wine installs its WoW64
 * CS through i386_set_ldt; its LDT_SIZE is 8192 and the entry typically lands
 * at index 32, which is selector (32 << 3) | TI | RPL = 0x107.
 */
#include "ocerz/vm.h"
#include "ocerz/mem.h"
...
#define SC_BSD_SIGACTION   0x0200002eull
#define SC_BSD_SIGRETURN   0x020000b8ull
...
static const uint64_t high_seed[8] = {
    ...
    0x00007ffe00001000ull,
    0x00007ffe00002000ull,
    STACK64,
...
int main(void)
{
    ...
    EMIT_AT(M64_CODE + 0x10, 0x41, 0xff, 0x2e);
...

    {
        ...
        EXPECT_U64("inv.dec32.opsize", 4, i32.opsize);
    ...
    seed_high();
    cpu()->gpr[OCERZ_RSP] = STACK32;
    ocerz_st(THUNK32 + 0, 4, M64_CODE + 0x40);
    ocerz_st(THUNK32 + 4, 2, CS64_SEL);
