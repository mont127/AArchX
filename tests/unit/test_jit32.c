/*
 * Differential JIT-vs-interpreter test for i386 (32-bit) blocks.
 *
 ...
 * through the interpreter, and once through the JIT, which compiles the whole
 * fragment into one arm64 block and runs it.  Then the entire architectural
 * state the fragment could have touched - all sixteen GPRs including their
 * high halves, the materialized RFLAGS, EIP, the guest mode, the xmm registers
 * and every byte of the data, stack and 4 GB-boundary windows - is compared.
 * A disagreement is a codegen bug; the interpreter is the oracle, and it is
 * the same interpreter the 16.8M-probe i386 decoder gate and
 * tests/unit/test_interp32.c already pin.
 *
 * That structure is why the cases are chosen the way they are.  They are not
 * "does ADD work" - ADD is width-driven and shared with 64-bit codegen.  They
 * are the five places where 32-bit mode is a DIFFERENT machine:
 *
 ...
 *      value mismatch, which the comparison catches, rather than a crash that
 *      could be mistaken for an unrelated fault.
 ...
 *
 * The last group is the other half of the claim: the opcodes the JIT refuses
 * to compile (PUSHA/POPA, the BCD adjusts, SALC, XCHG, BTS) must still run
 * through the interpreter callout and leave the same state - including when
 * they are interleaved with compiled instructions in one block.
 */
#include "ocerz/cpu.h"
...
#define ALIAS      0x100500000ull
#define WRAP_PAGE  0xfffff000ull
#define CS32       0x0fu

#define DATA_WIN   0x1000
...
int main(void)
{
    ...
    *(uint8_t *)ocerz_g2h(DONE) = 0xf4;
...

    {
        ...
        cal[0] = 0x83; cal[1] = 0xc0; cal[2] = 0x05;
        cal[3] = 0xc3;
    ...
    {
        uint8_t *cal = (uint8_t *)ocerz_g2h(M32_CODE + 0x100);
        cal[0] = 0xc2; cal[1] = 0x08; cal[2] = 0x00;
        static const uint8_t c[] = {
            0x68, 0xaa, 0xbb, 0xcc, 0xdd,
            0x68, 0x55, 0x66, 0x77, 0x88,
            0xe8, 0xf1, 0x00, 0x00, 0x00,
            0x83, 0xc0, 0x09,
        };
        run_case("ret imm16", M32_CODE, M32_STACK, c, sizeof c);
    }
    {
        int32_t rel = (int32_t)(uint32_t)(M32_CODE + 0x100);
        g_wrapcode[0] = 0xe8;
