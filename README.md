...
| native-mode gate (`-native`) | 78 / 78 |
| native-mode unit suites: API database, image, bridge, ABI, callbacks, thread attach, Objective-C | 308 / 0, 140626 / 0, 1227 / 0, 28028 / 0, 131974 / 0, 270 / 0, 123713 / 0 |
| real macOS apps opening their main window | 9 in cache mode, 1 in native mode (see [Application compatibility](#application-compatibility)) |
...
- An opt-in native mode that runs Intel programs against synthesized x86 system images bridged to the host's own arm64 libSystem, CoreFoundation, CoreGraphics, Objective-C runtime, Foundation and AppKit, in both directions, without the x86 shared cache. It runs command-line C and Objective-C programs, and Cocoa applications whose calls stay inside what it bridges, such as Image Capture.

## Application compatibility

Everything in this section runs in cache mode. In native mode Image Capture opens its main window; what stops the other system applications there is listed under [Native mode](#native-mode).
...
- **Frameworks without a database.** Only libSystem, libncurses, CoreFoundation, CoreGraphics, libobjc, Foundation and AppKit are synthesized, so a program linking any other library fails to bind and names every import it could not bind, and `dlopen` of one, or of an arm64 library, answers NULL with `native library without an API database`. Most system applications link more: TextEdit stops at UniformTypeIdentifiers and the Swift runtime, Dictionary at CoreServices, WebKit and Carbon, and Chess at OpenGL, Metal, GameKit and a dozen others.
- **Blocks.** A block written in x86 code cannot be handed to native code, so `dispatch_once`, `dispatch_async`, completion handlers and `enumerateObjectsUsingBlock:` stop. The `_f` function-pointer forms of libdispatch work. Stickies binds, loads its main nib and stops at its first `dispatch_once`.
...
- **Programs a guest starts.** Every program a guest starts runs under ocerz in native mode, so one whose x86_64 slice links a library native mode does not synthesize stops the way any such program does; among the system tools a shell command might run, `wc` links libxo and `sort` calls `fwprintf`. A program with no x86_64 slice is refused, as it is in cache mode. `forkpty`, `daemon` and `wordexp` are stubs, and a `fork` inside a callback that translated code made is refused.
- **Variadic calls without a veneer.** The `scanf` family, `syslog`, `err` and `warn` are stubs; the printf family, `NSLog`, CoreFoundation's format functions and Foundation's variadic methods work, and so do `open`, `fcntl`, `ioctl` and the other calls whose optional argument is fixed.
...
- Native mode runs command-line programs and some applications; the list under [What native mode cannot run yet](#native-mode) says what stops the rest. Beyond that list:
  ...
  - `mach_vm_map` and `mach_vm_remap` are stubs, and `msync`, `mlock` and `minherit` reach the host kernel with the guest's address unchanged, as they do in cache mode;
