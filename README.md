...
- **Frameworks without a database.** libSystem, libncurses, libutil, libz, CoreFoundation, CoreGraphics, libobjc, Foundation, AppKit, CFNetwork, SystemConfiguration, Security and CoreServices, ApplicationServices, CoreMedia, CoreVideo, VideoToolbox, CoreAudio, AudioToolbox, OpenAL, IOKit, DiskArbitration, Carbon, Cocoa, IOSurface, Accelerate, CoreHaptics, ForceFeedback, GameController, Metal, QuartzCore, UserNotifications, OpenGL, CoreText, IOBluetooth, libbz2 and libiconv have databases. Other system libraries still need one, and unsupported exports within these databases remain stubs. Missing imports are named; `dlopen` of a native library without a database answers NULL with `native library without an API database`. Swift, WebKit and GameKit remain among the gaps for larger applications.
