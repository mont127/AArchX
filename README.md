...
- Native mode runs only programs whose system calls stay inside the bridged part of libSystem, libdispatch and CoreFoundation. Nothing above that is available there yet, Foundation and AppKit included; variadic functions such as `printf`, `open` and `CFStringCreateWithFormat` are not bridged, nor are functions taking a structure by value, such as those taking a `CFRange`; a guest that is not position-independent runs with its low addresses shadowed, so a structure it hands CoreFoundation holding pointers into its own image would carry addresses native code cannot read; `dlopen` is not available there; a C++ program fails to bind, because no x86 `libc++` is synthesized; and a program reading `environ`, `stdout` or the ctype tables directly fails to bind.
