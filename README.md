...
Native mode reads its API database from `runtime/apis` beside the `ocerz` binary. The database is derived from the macOS SDK, so it is not in the repository: `make apis` generates it from your own SDK with the Command Line Tools' libclang, and `make check` runs that step itself. Run native mode from the source tree or keep that directory next to a copied binary:
...
| Brawlhalla | works (2026-09-22, owner-confirmed): the Adobe AIR game reaches its menus against a running Steam client. Needs the System V shared memory calls Steam's IPC uses and a working GPU, both new that day. Measured the same day against Rosetta with one Steam client for both: first frame in 5.3 s against 0.54 s, and its main thread saturates one core where Rosetta uses a quarter to half of one. Two fixes that evening: return-address-stack misses no longer cascade through the block epilogue, and a chain whose `b` cannot reach its target (more than 128 MB of translated code between a caller and a callee) goes through a veneer instead of being dropped. A third change resolves the commpage guard and the ordered-access alignment test at translation time for rip-relative and absolute operands, so a global load or store is four host words rather than fourteen or eighteen. The thread went from a saturated core to about three quarters of one while doing 75% more work per second, and at the menu it now spends most of its time waiting on the display, as under Rosetta. |
