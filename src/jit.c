...
static struct { uint32_t *site; uint64_t taken; int idx; uint32_t *stub; uint32_t *patch_b;
                ...
                uint16_t l0_dirty; uint16_t yc_dirty; } g_side[SIDE_MAX];
static int g_n_side;
...
static void emit_slowcall_keep_lanes(A64Buf *b, const X86Insn *insn, uint32_t **exit_sites, int *n_exits);
static void yc_reload_all(A64Buf *b);
...
static void undo_save_hook(A64Buf *b, int size, int vd)
{
...
static void fpb_replay_prelude(A64Buf *b, const FpBatch *fb, const int8_t *l0, const uint8_t *l0_dbl)
{
    for (int r = 0; r < 16; r++)
...
static uint16_t g_l0_owners[L0_NLANES];
...
static int8_t g_yc[16];
static uint16_t g_yc_dirty;
static void yc_flush_all(A64Buf *b)
{
    for (unsigned r = 0; r < 16; r++)
...
static void yc_flush_from(A64Buf *b, uint16_t dirty)
{
    for (unsigned r = 0; r < 16; r++)
...
static uint16_t g_l0_fixed_dirty;
static void l0_fixed_map(void)
{
    g_l0_dirty = 0;
...
static void fpb_emit_undo_save(A64Buf *b, const X86Insn *insn, int i, uint32_t **exit_sites, int *n_exits)
{
...
static void fpb_emit_undo_restore(A64Buf *b, const X86Insn *insns, int first, int end, uint32_t **exit_sites, int *n_exits)
{
...
static int yc_setup(A64Buf *b, const X86Insn *insns, int n)
{
    static int dis = -1;
...
static void l0_fixed_fallthrough(A64Buf *b)
{
    if (g_l0_fixed)
        l0_flush_all(b);
}
...
static void emit_l0_flush_from(A64Buf *b, const int8_t *l0, const uint8_t *l0_dbl, uint16_t dirty)
{
    for (unsigned r = 0; r < 16; r++)
...
static void emit_l0_reload_from(A64Buf *b, const int8_t *l0, const uint8_t *l0_dbl)
{
    for (unsigned r = 0; r < 16; r++)
        if (l0[r] >= 0) {
