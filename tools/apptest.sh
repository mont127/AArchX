...
#
# There is no timeout(1) on a stock macOS box, hence run_bounded below; the
# guest test scripts carry the same fallback.
#
...
#
# uname is expected to differ: an x86_64 process must be told it is x86_64,
# so the machine field reads x86_64 under ocerz and arm64 natively.  The
# runner knows about that one case and does not count it as a failure.
set -u
cd "$(dirname "$0")/.."
OCERZ=./ocerz
SECS="${SECS:-20}"
...
run_bounded() {   # run_bounded <secs> <cmd...>; returns 137 when killed
    local s=$1; shift
    "$@" & local p=$!
...
cli_one() {
    ...
    if [ "$bin" = /usr/bin/uname ]; then          # see the header
